...
const StagingOL = () => {
    ...
    const handleConfirm = async () => {
        ...
        if (window.confirm("¿Confirmar Orden de Liquidación y generar facturas?")) {
            ...
            try {
                const result = await confirmLiquidation(stagingLiquidation.pdfFile);
                if (!result?.success) {
                    // Store already alerts, but we log here just in case specific UI handling is needed
                    console.warn("Confirm failed:", result?.error);
                }
            } catch (e) {
                console.error("Unexpected error in StagingOL:", e);
                alert("Error inesperado en la vista de staging.");
            } finally {
